...
export function ConsultasView({ pets }: { pets: ClinicalPet[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return pets.filter((pet) => !query || [pet.name, pet.tutorName, pet.breed, pet.species].some((value) => value?.toLowerCase().includes(query)));
  }, [pets, search]);
  const selectedTimeline = useMemo(() => {
    ...
    return [...clinical, ...appointments].filter((item) => { const date = new Date(item.date); return date.getMonth() + 1 === Number(month) && date.getFullYear() === Number(year); }).sort((a, b) => b.date.localeCompare(a.date));
  }, [selected, month, year]);
  ...
  return <div className="space-y-5">
    ...
    <div className="relative max-w-xl"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar pet, tutor, espécie ou raça" className="pl-9" /></div>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((pet) => <button key={pet.id} type="button" onClick={() => setSelected(pet)} className="text-left"><Card className="h-full border-zinc-200 bg-white shadow-none transition hover:border-zinc-400"><CardContent className="p-5"><div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-lg bg-zinc-100"><PawPrint className="size-5" /></div><div className="min-w-0"><p className="truncate font-semibold">{pet.name}</p><p className="truncate text-xs text-zinc-500">{pet.species}{pet.breed ? ` · ${pet.breed}` : ""}</p></div></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><span className="rounded-md bg-zinc-50 p-2">{pet.appointments.length} atendimentos</span><span className="rounded-md bg-zinc-50 p-2">{pet.entries.length} registros</span></div><p className="mt-3 truncate text-xs text-zinc-500">Tutor: {pet.tutorName ?? "Não informado"}</p></CardContent></Card></button>)}</div>
    ...
    <Dialog open={selected !== null} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{selected?.name} · ficha do pet</DialogTitle></DialogHeader>{selected ? <>
      ...
      <div className="grid gap-3 sm:grid-cols-2"><div><Label>Mês</Label><select value={month} onChange={(event) => setMonth(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-zinc-200 bg-white px-3"><option value="1">Janeiro</option><option value="2">Fevereiro</option><option value="3">Março</option><option value="4">Abril</option><option value="5">Maio</option><option value="6">Junho</option><option value="7">Julho</option><option value="8">Agosto</option><option value="9">Setembro</option><option value="10">Outubro</option><option value="11">Novembro</option><option value="12">Dezembro</option></select></div><div><Label>Ano</Label><Input type="number" min={2020} max={2100} value={year} onChange={(event) => setYear(event.target.value)} /></div></div>

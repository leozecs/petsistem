...
/**
 * Defaults exist only before a tenant saves any schedule. Once one or more
 * explicit rows exist, they are authoritative — missing/inactive weekdays stay
 * closed instead of silently receiving 08:00-18:00.
 */
export function effectiveSchedules(schedules: ScheduleInput[]): ScheduleInput[] {
  return schedules.length === 0 ? DEFAULT_SCHEDULES : schedules;
}
